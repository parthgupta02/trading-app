            ...
            {isMobile ? (
                <div className="space-y-4">
                    {tradesWithWeek.length === 0 ? (
                        <div className="text-center text-xs text-gray-500 py-6">No trades recorded yet.</div>
                    ) : (
                        tradesWithWeek.map((trade) => (
                            <div key={trade.id} className="bg-[#111827] p-4 rounded-lg border border-gray-800 space-y-3">
                                <div className="flex justify-between items-start">
                                    <div className="flex items-center gap-2">
                                        <span className={`text-xs font-bold ${commodity === 'gold' ? 'text-[#F59E0B]' : 'text-gray-300'}`}>
                                            W{trade.weekNum}
                                        </span>
                                ...
                                <div className="grid grid-cols-3 gap-2">
                                    <div className="flex flex-col">
                                        ...
                                        <span className="text-sm font-medium text-gray-200">{trade.quantity || 1}</span>
                                    </div>
                                    <div className="flex flex-col">
                                        <span className="text-[10px] text-gray-500 uppercase">Buy</span>
                                        <span className={`text-sm font-medium ${Number(trade.buyAmount) > 0 ? 'text-green-500' : 'text-gray-600'}`}>
                                    ...
                                    <div className="flex flex-col text-right">
                                        ...
                                        <span className={`text-sm font-medium ${Number(trade.sellAmount) > 0 ? 'text-red-500' : 'text-gray-600'}`}>
                                ...
                                <div className="flex justify-end gap-2 pt-2 border-t border-gray-800">
                                    <Button variant="primary" onClick={() => setEditingTrade(trade)} className="px-3 py-1 text-xs h-8">Edit</Button>
                                    <Button variant="danger" onClick={() => setDeletingTradeId(trade.id as string)} className="px-3 py-1 text-xs h-8">Delete</Button>
                                </div>
                            </div>
                        ))
                    )}
